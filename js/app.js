...
class KHAgrifarmApp {
  ...
  async init() {
    ...
    // Set version badge
    const appVer = window.APP_CONFIG?.version || 'v10.75';
    const verFoot = document.getElementById('app-version-footer');
    if (verFoot) verFoot.innerText = appVer;
  ...
  setupEventListeners() {
    ...
    const modelSelect = document.getElementById('select-weather-model');

    if (modelSelect) {
      modelSelect.addEventListener('change', async (e) => {
        const newModelId = e.target.value;
        if (this.weatherService) {
          this.weatherService.setActiveModel(newModelId);
          const modelInfo = this.weatherService.getActiveModel();
          const modelDesc = document.getElementById('weather-model-desc');
          const modelSub = document.getElementById('settings-weather-model-subtitle');
          if (modelDesc) modelDesc.innerText = modelInfo.desc;
          if (modelSub) modelSub.innerText = `Active: ${modelInfo.name} (${modelInfo.origin})`;
          
          this.showToast(`🌤️ Weather switched to ${modelInfo.name}!`);
          await this.refreshWeather();
          this.updateActiveModelUI(newModelId);
          this.renderAll();
        }
      });
    }

    // 8b. On-Screen Quick Weather Model Dropdown
    const quickModelSelect = document.getElementById('select-weather-model-quick');
    if (quickModelSelect) {
      quickModelSelect.addEventListener('change', async (e) => {
        ...
        if (this.weatherService) {
          ...
          this.updateActiveModelUI(newModelId);
          const modelInfo = this.weatherService.getActiveModel();
          this.showToast(`🌤️ Switched to ${modelInfo.name}!`);
          await this.refreshWeather();
          this.renderAll();
        }
      });
    ...
    const modelBadgeClick = document.getElementById('weather-active-model-badge');
    if (modelBadgeClick) modelBadgeClick.addEventListener('click', openModal);
  ...
  populateSettingsModal() {
    ...
    const syncTimeStr = lastSyncTime ? `Synced: ${lastSyncTime}` : "Synced: Live";
    ...
    const appVer = window.APP_CONFIG?.version || 'v10.75';
    const verFoot = document.getElementById('app-version-footer');
    const verSet = document.getElementById('app-version-settings');
    const verDet = document.getElementById('app-version-detail');
    if (verFoot) verFoot.innerText = appVer;
    if (verSet) verSet.innerText = appVer;
    if (verDet) verDet.innerText = appVer;
    ...
    // 4. Weather Model Switcher Status
    if (this.weatherService) {
      const activeModel = this.weatherService.getActiveModel();
      const modelSelect = document.getElementById('select-weather-model');
      const modelDesc = document.getElementById('weather-model-desc');
      const modelSub = document.getElementById('settings-weather-model-subtitle');
      if (modelSelect) modelSelect.value = activeModel.id;
      if (modelDesc) modelDesc.innerText = activeModel.desc;
      if (modelSub) modelSub.innerText = `Active: ${activeModel.name} (${activeModel.origin})`;
    }
  ...
  updateActiveModelUI(modelId) {
    if (!this.weatherService) return;
    const activeModel = this.weatherService.getActiveModel();
    const curId = modelId || activeModel.id;

    // 1. Update on-screen quick dropdown
    const quickSel = document.getElementById('select-weather-model-quick');
    if (quickSel) quickSel.value = curId;

    // 2. Update Settings dropdown & description
    const sel = document.getElementById('select-weather-model');
    const desc = document.getElementById('weather-model-desc');
    const sub = document.getElementById('settings-weather-model-subtitle');
    if (sel) sel.value = curId;
    if (desc) desc.innerText = activeModel.desc;
    if (sub) sub.innerText = `Active: ${activeModel.name} (${activeModel.origin})`;
  }
  ...
  renderWeatherDetailView() {
    ...
    // 1. Google Maps Link & Coordinates & Model UI

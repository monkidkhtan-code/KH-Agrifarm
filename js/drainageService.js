...
class DrainageService {
  constructor(config) {
    ...
    this.cacheKey = this.config?.storageKeys?.drainageData || "kh_agrifarm_drainage_cache_v1";
    this.lastSyncKey = this.config?.storageKeys?.drainageLastSync || "kh_agrifarm_drainage_last_sync_v1";
    this.records = [];
    this.chartInstances = {};
  ...
  getLatestRecord(plotId) {
    const list = this.getPast5Records(plotId);
  ...
  renderPlotDrainageCard(plotId) {
    ...
    const ecInVal = summary && summary.ecIn !== null ? summary.ecIn.toFixed(1) : '--';
    const phInVal = summary && summary.phIn !== null ? summary.phIn.toFixed(1) : '--';
    const drainEcVal = summary && summary.avgDrainageEc !== null ? summary.avgDrainageEc.toFixed(1) : '--';
    const drainPhVal = summary && summary.avgDrainagePh !== null ? summary.avgDrainagePh.toFixed(1) : '--';
    const deltaVal = summary && summary.ecDelta !== null 
      ? (summary.ecDelta > 0 ? `+${summary.ecDelta.toFixed(1)}` : `${summary.ecDelta.toFixed(1)}`) 
      : '--';

    // Clean Lightweight Status Text (No Box Borders)
    let ecStatusText = '';
    if (summary && summary.ecDelta !== null) {
    ...
    let phStatusText = '';
    if (summary && summary.avgDrainagePh !== null) {
      const phColorClass = summary.avgPhEval.status === 'safe' ? 'text-emerald' : (summary.avgPhEval.status === 'danger' ? 'text-rose' : (summary.avgPhEval.status === 'warning' ? 'text-orange' : 'text-amber'));
    ...
    if (summary && summary.stations && summary.stations.length > 0) {
      stationTilesHtml = summary.stations.map(st => {
        const ecStr = st.ec !== null ? st.ec.toFixed(1) : '--';
        const phStr = st.ph !== null ? st.ph.toFixed(1) : '--';
        const ev = st.phEval;
        const tileClass = ev.status === 'safe' ? 'tile-safe' : (ev.status === 'danger' ? 'tile-danger' : (ev.status === 'warning' ? 'tile-warning' : 'tile-caution'));

        return `
          <div class="st-col-tile ${tileClass}" title="${st.name}: ${ecStr} EC, pH ${phStr} (${ev.tag})">
            <div class="st-tile-head">
              <span class="st-dot ${ev.dotClass}"></span>
              <span class="st-tile-name">${st.name.replace('Station ', 'St ')}</span>
            </div>
    ...
    if (summary && summary.hasStationWarning) {
      const names = summary.stationWarnings.map(w => `${w.name.replace('Station ', 'St ')} (pH ${w.ph})`).join(', ');
      warningBannerHtml = `
    ...
    return `
      <div class="activity-section drainage-monitoring-section" data-plot-id="${plotId}">
        ...
        <!-- 2 SEPARATE GRAPHS (EC TREND & PH TREND) -->
        <div class="drainage-graphs-card">
          <!-- Graph 1: 5-Day EC Run-Off Dynamics -->
  ...
  /**
   * Render All 5-Day Separate Charts (EC & pH) using Chart.js
   */
  renderAllDrainageCharts() {
    if (!window.Chart) return;

    ['plot-1', 'plot-2'].forEach(plotId => {
      const past5 = this.getPast5Records(plotId);
      const ordered = [...past5].reverse();

      let chartDataPoints = ordered.map(item => {
        const s = item.summary;
        return {
          label: s.date ? s.date.slice(0, 5) : 'Log',
      ...
      if (chartDataPoints.length === 0) {
        chartDataPoints = [
          { label: 'Baseline', ecIn: 0.2, ecDrain: 1.7, phDrain: 6.7 }
        ];
      }

...
class WeatherService {
  ...
  /**
   * Get the active weather model configuration
   */
  getActiveModel() {
    const saved = localStorage.getItem('kh_agrifarm_weather_model');
    const modelId = saved || this.config.weather?.primaryModel || "ecmwf_ifs025";
    const models = this.config.weather?.availableModels || [];
    const found = models.find(m => m.id === modelId);
    return found || {
      id: modelId,
      name: modelId === 'best_match' ? 'Best Match (Ensemble)' : modelId.toUpperCase(),
      origin: 'Global Model',
      desc: 'High-resolution meteorological forecast model.'
    };
  }

  /**
   * Set and persist the active forecast model
   */
  setActiveModel(modelId) {
    if (!modelId) return;
    localStorage.setItem('kh_agrifarm_weather_model', modelId);
    console.log(`[Weather] Forecast model switched to: ${modelId}`);
  }

  /**
   * Fetch 4-day hourly microclimate data for Tanjong Karang using the active precision model
   */
  ...
  formatWeatherData(data) {
    ...
    for (let i = 0; i < hourly.time.length; i++) {
      ...
      const rawProb = hourly.precipitation_probability ? hourly.precipitation_probability[i] : null;
      const rawRain = hourly.precipitation ? hourly.precipitation[i] : 0;
      let effectiveProb = 0;
      if (rawProb !== null && rawProb !== undefined && !isNaN(rawProb)) {
        effectiveProb = Math.round(rawProb);
      } else if (rawRain > 0) {
        effectiveProb = Math.min(Math.round(rawRain * 30) + 20, 95);
      }
    ...
    return {
      ...
      rainProbabilityNow: (next12Hours[0] && next12Hours[0].prob !== undefined && next12Hours[0].prob !== null) ? Math.round(next12Hours[0].prob) : 0,
